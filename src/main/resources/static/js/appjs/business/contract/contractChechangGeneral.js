		...
		.bootstrapTable(
			{
				...
				url : prefix + "/chechangList", // 服务器数据的加载地址
				...
				columns : [
					[{
						...
						formatter : function(value, row, index) {
							var e = '<a class="btn btn-primary btn-sm ' + s_contractChechangPay_h + '" href="#" mce_href="#" title="撤场缴费" onclick="contractChechangPay(\''
								+ row.id
								+ '\')"><i class="fa fa-edit"></i></a> ';
							var g = '<a class="btn btn-danger btn-sm ' + s_contractChechangCancel_h + '" href="#" title="撤场撤销"  mce_href="#" onclick="contractChechangCancel(\''
								+ row.id
				...
				onDblClickRow: function (item, $element) {
					...
					var perContent = layer.open({
						...
						area : [ '820px', '500px' ],
						content : prefix + '/contractDetail?contractCode='+item.contractCode+'&&oid='+item.oid // iframe的url
					});
...
function contractChechangPay(id) {
	layer.open({
		type : 2,
		title : '撤场缴费',
		maxmin : true,
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '520px' ],
		content : prefix + '/contractChechangPay/' + id// iframe的url
	});
}
function remove(id) {
...
function contractChechangCancel(id) {
	layer.confirm('确定要撤销选中的记录？', {
		btn : [ '确定', '取消' ]
	}, function() {
		$.ajax({
			url : prefix+"/contractChechangCancel",
			type : "post",
			data : {
				'id' : id
			},
