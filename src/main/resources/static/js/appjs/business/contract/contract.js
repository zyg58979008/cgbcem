				...
				columns : [
					[{
						...
						formatter : function(value, row, index) {
							...
							var g = '<a class="btn btn-warning btn-sm ' + s_contractChechang_h + '" href="#" title="撤场"  mce_href="#" onclick="contractChechangConfirm(\''
								+ row.id
								+ '\')"><i class="fa fa-remove"></i></a> ';
							return e + d + f + g;
						}
...
function contractChechangConfirm(id) {
	layer.open({
		type : 2,
		title : '撤场',
		maxmin : true,
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '520px' ],
		content : prefix + '/contractChechangConfirm/'+id // iframe的url
	});
}
function edit(id) {
	layer.open({
