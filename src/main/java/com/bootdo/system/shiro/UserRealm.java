...
/*	@Autowired
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		RoleDao roleDao=ApplicationContextRegister.getBean(RoleDao.class);
		List<Long> roles=roleDao.listRole(user.getUserId());
		if(roles.size()>0){
			user.setRoleIds(roles);
		}
		DeptDao deptDao = ApplicationContextRegister.getBean(DeptDao.class);
		DeptDO deptDO=deptDao.get(user.getDeptId());
