...
@Controller
@RequestMapping("/wuye/wuyefeiConfig")
public class WuyefeiController extends BaseController {
	private String prefix = "wuye/wuyefeiConfig";
	@Autowired
	private RoomService roomService;
	@Autowired
	private WuyefeiService wuyefeiService;
	@GetMapping()
	@RequiresPermissions("wuye:wuyefeiConfig:wuyefei")
	String room() {
	...
	@ResponseBody
	@GetMapping("/listNotHas")
	public PageUtils listNotHas(@RequestParam Map<String, Object> params) {
		//查询列表数据
		UserDO userDO= ShiroUtils.getUser();
		Query query = new Query(params);
		query.put("deptId",userDO.getDeptId());
		query.put("delFlag","0");
		List<RoomDO> roomList = roomService.listNotHas(query);
		int total = roomService.countListNotHas(query);
		PageUtils pageUtils = new PageUtils(roomList, total);
		return pageUtils;
	}
	@GetMapping("/add")
	@RequiresPermissions("wuye:wuyefeiConfig:add")
	...
	@GetMapping("/edit/{id}")
	@RequiresPermissions("wuye:wuyefeiConfig:edit")
